...
model_path = "results/deberta-large_prompt-based-balanced/checkpoint-168"
...
class DebertaForMaskedPromptBasedLM(transformers.DebertaForMaskedLM):
    def __init__(self, config):
        ...
        self.cls.predictions.decoder = nn.Linear(
            self.cls.predictions.embedding_size, config.num_labels, bias=False
        )
        self.cls.predictions.bias = nn.Parameter(torch.zeros(config.num_labels))
...
model = DebertaForMaskedPromptBasedLM.from_pretrained(
...
if __name__ == "__main__":
    ...
    training_args = transformers.TrainingArguments(
        ...
        report_to="wandb",
        # report_to="none",
        run_name="deberta-mlm-large_prompt-based-balanced",
